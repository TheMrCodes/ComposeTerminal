...
public abstract class Emulator {
    ...
    */
    ...
    protected void scroll_reverse() {
        term.draw_cursor();
        term.scroll_area(0, region_y1 - 1, term_width * char_width, region_y2 - region_y1, 0, 1);
        term.clear_area(x, y - 1, term_width * char_width, y);
        term.redraw(0, 0, term_width * char_width, term_height - 1);
        //term.setCursor(x, y);
        term.draw_cursor();
    }
    ...
    protected void scroll_forward() {
        term.draw_cursor();
        term.scroll_area(0, region_y1 - 1, term_width * char_width, region_y2 - region_y1 + 1, 0, -1);
        term.clear_area(0, region_y2 - 1, term_width * char_width, region_y2);
        term.redraw(0, region_y1 - 1, term_width * char_width, region_y2 - region_y1 + 1);
        term.draw_cursor();
    }
    ...
    protected void clr_eol() {
        term.draw_cursor();
        term.clear_area(x, y - 1, term_width * char_width, y);
        term.redraw(x, y - 1, (term_width) * char_width - x, 1);
        term.draw_cursor();
    }

    protected void clr_bol() {
        term.draw_cursor();
        term.clear_area(0, y - 1, x, y);
        term.redraw(0, y - 1, x, 1);
        term.draw_cursor();
    }

    protected void clr_eos() {
        term.draw_cursor();
        term.clear_area(x, y - 1, term_width * char_width, term_height);
        term.redraw(x, y - 1, term_width * char_width - x, term_height - y + 1);
        term.draw_cursor();
    }
    ...
    protected void draw_text() throws IOException {
        ...
        if ((b & 0x80) != 0) {
            ...
            pushChar(b);
            int foo = getASCII(term_width - x);
            if (foo != 0) {
                //System.out.println("foo="+foo+" "+x+", "+(y-1)+" "+(x+foo*char_width)+" "+y+" "+buf+" "+bufs+" "+b+" "+buf[bufs-foo]);
                //System.out.println("foo="+foo+" ["+new String(buf, bufs-foo, foo));
                term.clear_area(x, y, x + foo, y);
    ...
    private void check_region() {
        ...
        if (y > region_y2) {
            ...
            term.draw_cursor();
            term.scroll_area(0, region_y1, term_width, (region_y2 - region_y1), 0, -1);
            term.clear_area(0, y - 1, term_width, y);
            term.redraw(0, 0, term_width, region_y2);
            term.setCursor(x, y);
            term.draw_cursor();
        }
    }
